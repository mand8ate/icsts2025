...
export async function GET() {
	try {
		...
		const englishForms = await db.englishForm.findMany({
			orderBy: {
				incrementId: "asc", // Add explicit ordering
			},
		});

		if (!englishForms || englishForms.length === 0) {
			return new NextResponse(
		...
		const csv = parser.parse(englishForms);

		const response = new NextResponse(csv, { status: 200 });
		response.headers.set("Content-Type", "text/csv");
		response.headers.set(
			"Content-Disposition",
			"attachment; filename=englishForms.csv"
		);
